	...
	<div id="home" class="banner"
		...
		<div class="banner-content">
			<div class="well well-sm" style="background-color: grey">
				<div class="row">
					<h3 class="display-4">All reports from ${start } to ${end }</h3>
								...
								<form class="well form-horizontal" action="addTestTo"
									...
									<table id="mytable" class="table table-bordred table-striped">
										...
										<thead>
											...
											<th class="text-left">resultOfReport</th>
											<th class="text-left">reportingDate</th>
										</thead>
										<tbody>
											<c:forEach begin="0" end="${nosearch}" var="par"
												items="${search}">
												<tr>
													<td><c:out value="${par.reportId }" /></td>
													<td><c:out value="${par.sampleId}" /></td>
													<td><c:out value="${par.t_ID}" /></td>
													<td><c:out value="${par.resultOfReport}" /></td>
													<td><c:out value="${par.reportingDate}" /></td>
												</tr>
											</c:forEach>
										</tbody>
									</table>
